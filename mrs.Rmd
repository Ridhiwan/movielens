...
edx_cor <- edx_smp %>% select(-title) # remove the title column
edx_cor$genres <- as.numeric(as.factor(edx_cor$genres)) # change the genre column to numeric factors
corrlt <- rcorr(as.matrix(edx_cor)) # create the correlation object
...
```
There is no significant correlation between the variables except for timestamp and movieId  which have a positive correlation.
