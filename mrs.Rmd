...
The improvement is very small from the third to the fourth model thus we have to take a different approach to make the results a bit better. We will try regularization on all the three parameters that we added to the simplest model in order to improve its performance.
...
The penalty constant tat gives the lowest error is 4.75, now we will check to see what is the value of the lowest error.

```{r model4, echo=FALSE}
fifth_model <- function(p){
     a_r <- train %>%
          group_by(movieId) %>%
          summarise(a_r = sum(rating - mu)/(n()+p))
     b_r <- train %>% 
...
fifth_model(4.75) # calling the function to calculate the error from the fifth model 
```
Now we have a bit better improvement to our models but it is still not very good.
